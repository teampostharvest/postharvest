...
graph TD
    subgraph Orchestrator
        A[cli.py / FastAPI Orchestrator]
    end

    subgraph Playwright Browser Context
        B[Isolated Browser Context & Session]
        C[Pre-Goto Response Listener]
        D[page.goto & Settle]
        E[Timeline Tab Switcher]
        F[Feed Sentinel Scroll Engine]
    end

    subgraph Transport & Frame Collector
        G[Raw GraphQL Payload Collector]
        H[Async Promise Drainer]
        I[Snapshot Assembly]
    end

    subgraph 4-Layer Extraction Engine
        J[Layer 1: Fast Path Known Shapes]
        K[Layer 2: Connection Discovery]
        L[Layer 3: Story Validation Engine]
        M[Layer 4: Deduplicated Normalization]
    end

    subgraph Completeness & Telemetry
        N[9-Stage Boundary Metrics Calculator]
        O[Completeness Contract Check]
        P[SourceResult Object]
    end
...
sequenceDiagram
    ...
    Engine->>Browser: Register response listener before goto
    Engine->>Browser: Navigate page.goto(url)
    Browser->>Net: Initial feed GET request
    Net-->>Browser: Initial HTML and GQL Payloads
    Browser-->>Engine: Intercept and retain GQL payloads

    loop Feed Acquisition Loop (Target 50 Posts)
        Engine->>Browser: Scroll Feed Sentinel
        Browser->>Net: POST /api/graphql/
        Net-->>Browser: GraphQL Story Fragments
        Browser-->>Engine: Intercept response update IDs
        Engine->>Engine: Check progress and reset stale counters
    end

    Engine->>Browser: Await Promise.all(pendingReads)
    Engine->>Parser: Pass Snapshot Payload
    Parser->>Parser: Execute 4-Layer Extraction
    Parser->>Engine: Return Canonical Validated Posts
    Engine->>Store: Calculate Boundary Metrics and Completeness Status
